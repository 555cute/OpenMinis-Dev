...
object ExecutionCoordinator {
    ...
    private suspend fun getOrCreateShell(sessionId: String): PersistentShell {
        ...
        if (existing != null && existing.isAlive) {
            Log.w(TAG, "[diag] reuse existing shell for sessionId=$sessionId attachmentsMount=${existing.debugBindMount("/var/minis-autocompact/attachments")}")
            return existing
        }
        ...
        return globalLock.withLock {
            ...
            if (recheck != null && recheck.isAlive) {
                Log.w(TAG, "[diag] reuse existing shell (post-lock) for sessionId=$sessionId attachmentsMount=${recheck.debugBindMount("/var/minis-autocompact/attachments")}")
            ...
            Log.w(TAG, "[diag] new shell created sessionId=$sessionId attachmentsMount=${bindMounts["/var/minis-autocompact/attachments"]}")
            shell
        }
    }
    ...
    private fun buildSessionBindMounts(sessionId: String): Map<String, String> {
        ...
        val prevAttachments = PRootKernel.bindMounts["/var/minis-autocompact/attachments"]

        // Session-specific directories
        val sessionBase = File(filesDir, "minis-autocompact-sessions/$sessionId")
        listOf("attachments", "offloads", "workspace", "browser").forEach { subdir ->
            val hostDir = File(sessionBase, subdir).also { it.mkdirs() }
            val linuxPath = "/var/minis-autocompact/$subdir"
        ...
        Log.w(TAG, "[diag] buildSessionBindMounts sessionId=$sessionId " +
            "attachments: prev=$prevAttachments new=${mounts["/var/minis-autocompact/attachments"]}")
        ...
        // shell that runs minis-mcp-cli — /var/minis-autocompact/mcp-servers/servers.json
        ...
        // below.
        val globalBase = File(filesDir, "minis-autocompact-global")
        ...
        // is invisible to the shell — `ls /var/minis-autocompact/mounts/<name>/` then shows
        ...
        PRootKernel.mountedFoldersStore?.entries?.value?.forEach { entry ->
            ...
            val linuxPath = "/var/minis-autocompact/mounts/${entry.name}"
