...
object ReadImageTool {
    ...
    fun definition(): AgentToolDefinition = AgentToolDefinition(
        ...
        parameters = mapOf(
            ...
            "path" to AgentToolParam("string", "Linux path (e.g. /var/minis-autocompact/attachments/chart.png) or minis-ac:// URL (e.g. minis-ac://attachments/chart.png)"),
        ),
     ...
     * (`/var/minis-autocompact/{attachments,workspace,offloads,browser}/...`) resolve
    ...
    fun execute(argsJson: String, sessionId: String? = null, context: Context? = null): ToolExecutionResult {
        return try {
            ...
            val path = if (rawPath.startsWith("minis-ac://")) {
                "/var/minis-autocompact/" + java.net.URLDecoder.decode(rawPath.removePrefix("minis-ac://"), "UTF-8")
            } else rawPath
