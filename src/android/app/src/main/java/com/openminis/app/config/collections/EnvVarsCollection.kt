 ...
 * `minis-ac://settings/environments?create_key=…` so the user supplies
 * the value directly.
 */
class EnvVarsCollection(
    ...
    override fun add(payload: ConfigValue): String {
        throw ConfigError.PermissionDenied(
            "Use the deep link `minis-ac://settings/environments?create_key=…` so the user enters the value directly"
        )
    }
